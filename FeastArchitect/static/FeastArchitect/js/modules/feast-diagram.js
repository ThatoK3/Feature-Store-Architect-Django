...
class FeastDiagram {
    ...
    _llmHighlightNodes(nodeIds) {
        ...
        this._featureHighlightIds = new Set(nodeIds);
        if (nodeIds.length === 1 && this.nodes.nodes.has(nodeIds[0])) {
            this.selectNode(nodeIds[0]);
        }
        this.showNotification('Highlighted', `${nodeIds.length} node${nodeIds.length !== 1 ? 's' : ''} highlighted`);
    }
    ...
    addLLMActionButtons(messageDiv) {
        // Legacy stub — action bars injected by LLMHelper._appendActionBar
    }
